...
- [Лабораторная работа №4 - РЕАЛИЗАЦИЯ_КЛИЕНТСКОЙ_ЧАСТИ_СРЕДСТВАМИ_Vue.js](#лабораторная-работа-4)
---
...
Овладеть практическими навыками разработки клиентской части web-приложения с использованием:

- Vue 3
- Vuetify
- Axios
- взаимодействия с REST API, реализованным на Django REST Framework.

---

## Описание работы

В рамках лабораторной работы была реализована клиентская часть информационной системы администратора гостиницы.

Frontend-приложение взаимодействует с серверной частью, разработанной в лабораторной работе №3.

Разработанный интерфейс позволяет:

- регистрироваться и авторизовываться пользователям;
- просматривать список номеров гостиницы;
- искать свободные номера по выбранным датам;
- бронировать номера;
- просматривать существующие бронирования;
- управлять учетной записью пользователя.

Клиентская часть реализована на **Vue 3** с использованием библиотеки компонентов интерфейса **Vuetify**.
...
В ходе реализации клиентской части были использованы следующие технологии.

### Vue 3
Использовался для разработки интерфейса приложения и организации архитектуры на основе компонентов.

### Vuetify
Библиотека UI-компонентов, применяемая для создания элементов интерфейса: карточек комнат, кнопок, форм ввода данных, навигационной панели.

### Axios
HTTP-клиент, используемый для отправки запросов к серверной части приложения и получения данных через REST API.

---

## Реализация взаимодействия с сервером

Для взаимодействия клиента с серверной частью были реализованы API-модули.
...
Главная страница позволяет пользователю выбрать даты проживания и получить список свободных номеров.

Каждый номер отображается в виде карточки, содержащей:
...
На странице бронирования пользователь может:

- создать нового гостя;

- выбрать номер;

- указать даты проживания;

- оформить бронирование.

После заполнения формы данные отправляются на сервер через REST API.

```javascript
await staysApi.create({
  guest: guestId,
  room: stay.value.room,
  check_in: stay.value.check_in,
  check_out: stay.value.check_out
});
```
...
Авторизация реализована через Token Authentication.
...
В системе реализована страница профиля пользователя, где можно:
...
Для создания пользовательского интерфейса использовалась библиотека Vuetify.
...
В приложении была применена цветовая тема с использованием оранжевого цвета для основных элементов интерфейса.

## Вывод

В ходе выполнения лабораторной работы была разработана клиентская часть web-приложения для системы управления гостиницей.
...
В процессе выполнения работы были закреплены навыки разработки frontend-приложений на Vue.js, работы с REST API и создания пользовательских интерфейсов с использованием библиотеки Vuetify.
