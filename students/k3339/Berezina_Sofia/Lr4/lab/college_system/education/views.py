...
from .serializers import (
    ...
    GradeListSerializer, GradeCreateSerializer, GradeUpdateSerializer, GroupDetailSerializer, GradeDeleteSerializer
)
...
# 3. Эндпоинт: Группы по курсу (или все группы)
class GroupsByCourseAPIView(APIView):
    """Получить группы по номеру курса или все группы если курс не указан"""
    permission_classes = [AllowAny]
...
class StudentDeleteAPIView(generics.DestroyAPIView):
    """Удалить студента (отчислить)"""
    queryset = Student.objects.all()
    permission_classes = [IsAdminUser]

    def perform_destroy(self, instance):
        # Вместо удаления меняем статус на "отчислен"
        instance.status = 'dropped'
        instance.save()

# 9. Эндпоинт: Поиск студентов
class StudentSearchAPIView(generics.ListAPIView):
    """Поиск студентов по имени/фамилии/ID"""
    serializer_class = StudentSerializer
    filter_backends = [filters.SearchFilter]
    ...
    def get_queryset(self):
        ...
        # Если передан параметр id, фильтруем по ID
        student_id = self.request.query_params.get('id', None)
        if student_id:
            try:
                queryset = queryset.filter(id=int(student_id))
...
class GradeDeleteAPIView(generics.DestroyAPIView):
    """Удалить оценку"""
    queryset = Grade.objects.all()
    serializer_class = GradeDeleteSerializer
    permission_classes = [IsAuthenticated]
    lookup_field = 'id'
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            print(f"Удаление оценки ID: {instance.id}")
            print(f"Студент: {instance.student}")
            print(f"Предмет: {instance.subject}")
            print(f"Оценка: {instance.grade}")

            # Можно добавить дополнительную логику перед удалением
            if instance.student.status != 'active':
                return Response(
                    {"error": "Нельзя удалить оценку неактивного студента"},
                    status=status.HTTP_400_BAD_REQUEST
                )
