...
from hotel_app.serializers import ClientSerializer, ReservationSerializer, EmployeeSerializer, \
    EmployeeScheduleSerializer, RoomSerializer, ReservationCreateSerializer, ReservationCreateWithAdminSerializer, \
    EmployeeScheduleUpdateSerializer, EmployeeScheduleCreateSerializer
...
class ClientsAPIView(GenericAPIView):
    ...
    def post(self, request):
        ...
        return Response({"Success": "Client '{}' created succesfully.".format(client_saved.full_name)})
...
class ReservationsAPIView(GenericAPIView):
    ...
    def post(self, request):
        ...
        serializer = ReservationCreateWithAdminSerializer(data=reservation)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
...
class EmployeesAPIView(GenericAPIView):
    ...
    def post(self, request):
        ...
        return Response({"Success": "Employee '{}' hired successfully.".format(employee_saved.full_name)})
...
class EmployeeDetailsAPIView(GenericAPIView):
    ...
    def get(self, request, pk):
        try:
            ...
            return Response({"Error": "Employee '{}' does not exist.".format(pk)})

    def delete(self, request, pk):
        try:
            employee = Employee.objects.get(pk=pk)
            employee.delete()
            return Response({"Success": "Employee '{}' deleted successfully.".format(pk)})
        except Employee.DoesNotExist:
            return Response({"Error": "Employee '{}' does not exist.".format(pk)})
...
class EmployeeSchedulesAPIView(GenericAPIView):
    permission_classes = [IsAuthenticated]
    serializer_class = EmployeeScheduleCreateSerializer
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            serializer.save()
        return Response({"Success": "Employee schedule created successfully."})
...
class ReservationDetailsView(GenericAPIView):
    ...
    def get(self, request, id):
        try:
            ...
            return Response({"Error": "Reservation with pk '{}' does not exist.".format(id)})

    def delete(self, request, id):
        try:
            reservation = Reservation.objects.get(pk=id)
            reservation.delete()
            return Response({"Success": "Reservation deleted successfully."})
        except Reservation.DoesNotExist:
            return Response({"Error": "Reservation with pk '{}' does not exist.".format(id)})
...
class FreeRoomsAPIView(GenericAPIView):
    ...
    def get(self, request):
        return Response(
            {"Count of free rooms: ": Reservation.objects.filter(departure_date__lte=datetime.now()).count()})
...
class ClientsFromCityAPIView(GenericAPIView):
    ...
    def get(self, request):
        ...
        return Response(
            {f"Count of clients from {city}: ": Client.objects.filter(city_from=city).count()})
...
class EmployeeFromClientRoomAPIView(GenericAPIView):
    ...
    @swagger_auto_schema(
        operation_description="Get employee who were cleaning client's room",
        manual_parameters=[
            openapi.Parameter(
                'client_id',
                openapi.IN_QUERY,
            ...
            openapi.Parameter(
                'day_of_week',
                openapi.IN_QUERY,
                description="The day when employee cleans the floor",
                type=openapi.TYPE_STRING,
                required=True,
            ),
        ],
        responses={
            200: openapi.Response(description="Successful retrieval", schema=EmployeeSerializer),
        }
    )
...
class ReservationClientsAPIView(GenericAPIView):
    ...
    @swagger_auto_schema(
        operation_description="Get information about clients, who were living in this room in specific period.",
        manual_parameters=[
            openapi.Parameter(
                'room_id',
                openapi.IN_QUERY,
...
class ClientByOtherClientAPIView(GenericAPIView):
    ...
    @swagger_auto_schema(
        operation_description="Get information about clients, who were living in the hotel in specific period with target client.",
        manual_parameters=[
            openapi.Parameter(
                'client_id',
                openapi.IN_QUERY,
        ...
        responses={
            200: openapi.Response(description="Successful retrieval", schema=ClientSerializer),
        }
    )
    ...
    def get(self, request):
        ...
        start_date = request.query_params.get('start_date')
        end_date = request.query_params.get('end_date')
        target_client_reservations = Reservation.objects.filter(arrival_date__lte=end_date,
                                                                departure_date__gte=start_date, client_id=client_id)
        clients = []
...
class ReportAPIView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="Retrieve report for specific quarter.",
        manual_parameters=[
            openapi.Parameter(
                'quarter',
                openapi.IN_QUERY,
    ...
    def get(self, request):
        ...
        print(quarter)
        if not (1 <= quarter <= 4):
            raise BadRequest("Введите корректный номер квартала (от 1 до 4)")
