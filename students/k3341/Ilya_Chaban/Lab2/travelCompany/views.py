...
def delete_tour(request, tour_id):
    hotel = get_object_or_404(TravelAgency, id=tour_id)
...
@login_required
def delete_room(request, firm_id, tour_id):
    tour = get_object_or_404(Tour, id=tour_id, agency_id=firm_id)  # Используйте правильное имя связанного поля
    if request.user.is_superuser:
        tour.delete()
        messages.success(request, 'Тур успешно удалён.')
    else:
        messages.error(request, 'У вас нет прав для удаления этого тура.')
    return redirect('tour_list', tour_id=firm_id)
