    ...
    return (
        <Container sx={{ mt: 4 }}>
            ...
            <Box sx={{ mb: 2 }}>
                ...
                <Typography variant="body1"><b>Статус:</b> {printshop.is_closed ? 'Закрыта' : 'Открыта'}</Typography>
                <Typography variant="body1"><b>Дата добавления:</b> {new Date(printshop.created_at).toLocaleString()}</Typography>
                <Typography variant="body1"><b>Дата обновления:</b> {new Date(printshop.updated_at).toLocaleString()}</Typography>
            </Box>
            <Button
                variant="contained"
                color="primary"
