...
func (c *Client) GetAllPrintingRuns(ctx context.Context) ([]*entities.PrintingRun, error) {
	query := `
		SELECT pr.id, pr.printing_house_id, pr.newspaper_id, pr.circulation,
		       ph.id, ph.name, ph.address, ph.is_active,
		...
		JOIN printing_houses ph ON pr.printing_house_id = ph.id
	...
	rows, err := c.pool.Query(ctx, query)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get all printing runs")
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&pr.ID, &pr.PrintingHouseID, &pr.NewspaperID, &pr.Circulation,
			&ph.ID, &ph.Name, &ph.Address, &ph.IsActive,
...
// GetAllDistributions retrieves all distributions
func (c *Client) GetAllDistributions(ctx context.Context) ([]*entities.Distribution, error) {
	query := `
	...
	rows, err := c.pool.Query(ctx, query)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get all distributions")
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&d.ID, &d.PostOfficeID, &d.NewspaperID, &d.PrintingHouseID, &d.Quantity,
			&po.ID, &po.Number, &po.Address,
			&n.ID, &n.Title, &n.PublicationIndex, &n.EditorFirstName, &n.EditorLastName, &n.EditorMiddleName, &n.PricePerCopy,
			&ph.ID, &ph.Name, &ph.Address, &ph.IsActive); err != nil {
