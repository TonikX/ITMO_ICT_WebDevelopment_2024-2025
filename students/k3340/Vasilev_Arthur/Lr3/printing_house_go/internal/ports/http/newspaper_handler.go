...
func (s *Server) handleGetNewspaperFullDetail(w http.ResponseWriter, r *http.Request) {
	...
	// Get printing runs for this newspaper
	printingRuns, err := s.printingRunService.GetPrintingRunsByNewspaperID(r.Context(), id)
	if err == nil {
		for _, pr := range printingRuns {
			...
			result.PrintingRuns = append(result.PrintingRuns, dto.PrintingRunNestedDTO{
				...
				PrintingHouse: dto.PrintingHouseDTO{
					ID:       pr.PrintingHouse.ID,
					Name:     pr.PrintingHouse.Name,
					Address:  pr.PrintingHouse.Address,
					IsActive: pr.PrintingHouse.IsActive,
				},
	...
	// Get distributions for this newspaper
	allDistributions, err := s.distributionService.GetAllDistributions(r.Context())
	if err == nil {
		for _, d := range allDistributions {
			if d.NewspaperID == id && d.PrintingHouse != nil {
				result.Distributions = append(result.Distributions, dto.DistributionNestedDTO{
					ID:       d.ID,
					Newspaper: dto.NewspaperDTO{
						ID:               n.ID,
						Title:            n.Title,
					...
					PrintingHouse: dto.PrintingHouseDTO{
						ID:       d.PrintingHouse.ID,
						Name:     d.PrintingHouse.Name,
						Address:  d.PrintingHouse.Address,
						IsActive: d.PrintingHouse.IsActive,
					},
					Quantity: d.Quantity,
				})
			}
