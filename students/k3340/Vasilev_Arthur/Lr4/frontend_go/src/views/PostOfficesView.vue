  ...
  <v-container class="py-8">
    <v-row class="mb-8">
      <v-col cols="12">
        <div class="d-flex align-center justify-space-between mb-4">
          <div>
            <h1 class="text-h3 font-weight-bold">Post Offices</h1>
            <p class="text-grey mt-2">Manage all postal distribution centers</p>
          </div>
          <v-btn
            color="accent"
            size="large"
      ...
      <v-card rounded="lg">
        <div class="dialog-header px-6 py-5" style="background: linear-gradient(135deg, #06B6D4 0%, #22D3EE 100%); color: white; border-radius: 8px 8px 0 0;">
          <h2 class="text-h5 font-weight-bold">
            {{ editingItem ? '✎ Edit Office' : '➕ New Office' }}
          </h2>
        </div>
        ...
        <v-card-actions class="px-6 py-4 justify-end gap-2">
          ...
          <v-btn color="accent" @click="save" :loading="saving" class="font-weight-bold">
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="detailsDialog" max-width="800px" rounded="lg">
      <v-card v-if="selectedItem" rounded="lg">
        <div class="dialog-header px-6 py-5" style="background: linear-gradient(135deg, #10B981 0%, #34D399 100%); color: white; border-radius: 8px 8px 0 0;">
          <h2 class="text-h5 font-weight-bold">📮 Office Details</h2>
        </div>

        <v-card-text class="px-6 py-5">
          <v-row>
            <v-col cols="12" md="6">
              <p class="text-grey text-body-2">OFFICE NUMBER</p>
              <p class="text-h6 font-weight-bold">{{ selectedItem.number }}</p>
          ...
          <div v-if="fullDetails">
            <h3 class="text-h6 font-weight-bold mb-4">📦 Distributions</h3>
            <v-table v-if="fullDetails.distributions?.length" class="custom-table">
              <thead>
                <tr>
                  <th>Publication</th>
                  <th>Quantity</th>
              ...
              <tbody>
                <tr v-for="dist in fullDetails.distributions" :key="dist.id">
                  <td><strong>{{ dist.newspaper.title }}</strong></td>
                  <td>{{ dist.quantity }}</td>
                </tr>
              </tbody>
            </v-table>
    ...
    <v-dialog v-model="deleteDialog" max-width="400px" rounded="lg">
      <v-card rounded="lg">
        ...
        <v-card-text class="px-6 py-5">
          <p>Are you sure you want to delete this post office?</p>
          <p v-if="itemToDelete" class="font-weight-bold text-h6 mt-4">
            Office: "{{ itemToDelete.number }}"
          </p>
        </v-card-text>
...
async function loadPostOffices(options?: TableOptions) {
  ...
  try {
    ...
    console.error('Error loading post offices')
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  try {
    ...
    console.error('Error saving post office')
  } finally {
    saving.value = false
  }
}
...
async function deleteItem() {
  ...
  try {
    ...
    console.error('Error deleting post office')
  } finally {
