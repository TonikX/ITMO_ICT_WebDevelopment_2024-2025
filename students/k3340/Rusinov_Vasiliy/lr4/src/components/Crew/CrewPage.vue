...
function getPaper(){
  instance.get(`/library/book-taken/${index.currentRoute.value.params.id}/`, {
    headers: {
...
function savePaper(){
  ...
  instance.patch(`/library/book-taken/${index.currentRoute.value.params.id}/`, rest, {
    headers: {
      'Authorization': `Token ${Token.token}`
    }
  }).then(response => {
        if (response.status === 200){
          index.push('/book-takes')
        }
      }
  ).catch(error => console.log(error))
}
