<template>
    <v-container>
      <v-row>
        <v-col cols="12">
          <h1 class="text-center">Редактировать договор №{{ contract.id }}</h1>
        </v-col>
      </v-row>
      ...
      <v-form ref="form">
        ...
        <v-btn color="primary" @click="updateContract">Сохранить</v-btn>
      </v-form>
    </v-container>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchContract() {
        try {
          const response = await apiClient.get(`/contracts/${this.$route.params.id}/`);
          this.contract = response.data;
          this.contract.client_id = this.contract.client.id;
          this.contract.auto_id = this.contract.auto.id;
          this.contract.employee_id = this.contract.employee ? this.contract.employee.id : null;
        } catch (error) {
          console.error("Ошибка загрузки договора:", error.response?.data || error.message);
        }
      },
      async updateContract() {
        try {
          const response = await apiClient.put(`/contracts/${this.contract.id}/`, this.contract);
          console.log("Успешно обновлен договор:", response.data);
          this.$router.push("/contracts");
        } catch (error) {
          console.error("Ошибка при обновлении договора:", error.response?.data || error.message);
        }
      },
    },
    async mounted() {
      await Promise.all([this.fetchChoices(), this.fetchContract()]);
    },
  };
  </script>
