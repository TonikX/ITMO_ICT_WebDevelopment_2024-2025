...
Все запросы были сделаны в цикле в соответствующем `task3.1.1.py` файле. 
...
А следующим кодом можно все запринтить и убедиться в том, что все запросы применились (исключаем первый id, т.к. просто захотел избежать логирование суперпользователя)
```python
print("Автовладельцы:")
...
Соответственно аналогичный листинг для всех будет выглядеть так:

![image](media/logs_task1.png)

В admin панеле тоже можно видеть новые изменения:

![image](media/admin_panel_task1.png)
...
Все запросы представлены в файле `task3.1.2.py`.
...
Пример логов от print представлен ниже:
![image](media/logs_task2.png)
...
Все запросы представлены в файле `task3.1.3.py`.
После делаем запросы:
