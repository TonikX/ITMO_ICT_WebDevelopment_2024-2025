...
interface MedicalCard {
  medicalCardId: number; // Changed from MedicalCardID to medicalCardId
  issueDate: string;     // Changed from IssueDate to issueDate
  notes?: string;
  patientId: number;     // Changed from PatientID to patientId
  patient: Patient;
}
...
function MedicalCards({ patientId, onClose }: MedicalCardsProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.post(
        "/medicalcards/",
        {
          issueDate: newIssueDate, // Changed to issueDate
          notes: newNotes,
          patientId: patientId,     // Changed to patientId
        },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access")}`,
  ...
  const handleEditOpen = (card: MedicalCard) => {
    setCurrentCard(card);
    setEditIssueDate(card.issueDate); // Changed to issueDate
  ...
  const handleEditSave = async () => {
    if (currentCard) {
      ...
      try {
        await api.put(
          `/medicalcards/${currentCard.medicalCardId}/`, // Changed to medicalCardId
          {
            issueDate: editIssueDate, // Changed to issueDate
            notes: editNotes,
            patientId: patientId,     // Changed to patientId
          },
          {
  ...
  const handleDelete = async (cardId: number) => {
    if (window.confirm("Are you sure you want to delete this medical card?")) {
      try {
        await api.delete(`/medicalcards/${cardId}/`, { // Changed to medicalCardId
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access")}`,
          },
        });
        setMedicalCards(medicalCards.filter((c) => c.medicalCardId !== cardId)); // Changed to medicalCardId
      } catch (error) {
