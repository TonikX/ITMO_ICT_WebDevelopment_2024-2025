...
const NewspaperForm: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEdit && newspaperId) {
                await api.put(`/newspapers/${newspaperId}/`, formData);
            } else {
                await api.post('/newspapers/', formData);
            }
            navigate('/newspapers');
        } catch (err) {
    ...
    const getEditorFullName = (editor: Editor): string =>
        `${editor.last_name} ${editor.first_name} ${editor.patronymic || ''}`.trim();
