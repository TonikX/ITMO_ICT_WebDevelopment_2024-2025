...
class RouteStatsViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == 'list':
            return RouteTimeSerializer  # Используем сериализатор для времени работы маршрутов
        return RouteTimeSerializer  # Можно добавить другие сериализаторы для других действий, если потребуется

    @action(detail=False, methods=['GET'])
    def total_duration(self, request):
        # Получение общей протяженности маршрутов
    ...
    def driver_class_count(self, request):
        # Получение количества водителей по классам
