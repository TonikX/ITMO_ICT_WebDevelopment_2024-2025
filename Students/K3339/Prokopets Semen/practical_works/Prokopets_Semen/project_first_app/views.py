...
def owners_list(request):
    owners = Owner.objects.all()  # Получаем всех владельцев из базы данных
...
class CarDetailView(View):
    def get(self, request, car_id):
        car = get_object_or_404(Car, pk=car_id)  # Получаем автомобиль по ID или 404
        return render(request, 'cars/car_detail.html', {'car': car})

class CarUpdateView(View):
    def get(self, request, car_id):
        car = get_object_or_404(Car, pk=car_id)
        form = CarForm(instance=car)  # Предполагается наличие формы
    ...
    def post(self, request, car_id):
        ...
        if form.is_valid():
            ...
            return redirect('car_detail', car_id=car.id)  # Перенаправляем на страницу деталей автомобиля
        return render(request, 'car/car_form.html', {'form': form})
...
def owner_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()  # Сохраняем нового владельца в базе данных
            return redirect('owner_list')  # Перенаправляем на страницу списка владельцев
    else:
        form = OwnerForm()
